...
class CandiboxSync < Thor
  ...
  method_option :secret, :aliases => "-t", type: :string,
  ...
  Generating new candibox keypair
    ...
    if (tries -= 1) > 0
      ...
      attributes = {
        "portal_hostname" => options[:host],
        "portal_port" => sample_attributes['portal_port'],
        "api_user" => options[:username],
        "secret" => options[:secret],
        "box_key" => options[:box_private_key] || sample_attributes['box_key'],
        "ldap_host" => nil,
        "ldap_port" => sample_attributes['ldap_port'],
